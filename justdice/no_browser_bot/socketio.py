#
# This is a modification of the original socketIO_client.__init__.py
#
...
class SocketIO(object):
    ...
    def __init__(self, host, origin, response, secure=False):
        """
        ...
        """
        self._socketIO = _SocketIO(host, secure, response, origin)
    ...
    def run(self):
        while not self.done.is_set():
            try:
                ...
                print error
                self.cancel()
                # Notify connection error. issue #18
...
class _SocketIO(object):
    ...
    def __init__(self, host, secure, response, origin):
        baseURL = '%s/socket.io/%s' % (host, PROTOCOL)
        ...
        websock.connect(socketURL, origin=origin)
        self.connection = websock
